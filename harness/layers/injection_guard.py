    ...
    def wrap_tool_call(self, ctx, call, name, args):
        ...
        while BLOCK_START in content:
            ...
            content = (
                ...
                if end == -1
                else content[:start] + PLACEHOLDER + content[end + len(BLOCK_END):]
            )
        return ToolResult(ok=result.ok, content=content, error=result.error)

    def after_agent(self, ctx, report):
        answer = report.get("answer")
        if not isinstance(answer, str) or INJECTION_CANARY not in answer:
            return report
        sanitized = report.copy()
        sanitized["answer"] = answer.replace(INJECTION_CANARY, "")
        return sanitized
