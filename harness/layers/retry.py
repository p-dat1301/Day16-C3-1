...
from arena.model import is_degraded  # noqa: F401  (used by wrap_tool_call)
...
class Retry(Middleware):
    ...
    def wrap_tool_call(self, ctx, call, name, args):
        ...
        while attempts < self.max_attempts:
            ...
            if "budget" in error and ("blocked" in error or "reserve" in error):
                break
            ...
            result = call(name, args)
            attempts += 1
        ctx.state.setdefault("retry_attempts", []).append(
            {"name": name, "args": args, "attempts": attempts}
        )
        return result
